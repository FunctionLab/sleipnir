...
#pragma warning( default : 4996 )
	...
	aWords = (WORD*)my_malloc( sizeof(WORD) * ( (*piWords) + 1 ) );
...
WORD	CSVMImpl::s_asWords[ CSVMImpl::c_iWords ];
...
DOC* CSVMImpl::CreateDoc( const SData& sData, size_t iOne, size_t iTwo, size_t iDoc ) const {
	...
	asWords = ( iWords >= c_iWords ) ? new WORD[ iWords + 1 ] : s_asWords;
	for( i = 0; i < iWords; ++i )
		asWords[ i ].wnum = i + 1;
	asWords[ i ].wnum = 0;
...
bool CSVM::Open( std::istream& istm ) {
	...
	asWords = new WORD[ m_pModel->totwords + 1 ];
