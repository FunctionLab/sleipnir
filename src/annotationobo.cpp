 ...
 */
    bool COntologyOBO::Open(std::istream &istmOntology, std::istream &istmAnnotations, CGenome &Genome,
                            COntologyOBO &OntoBP, COntologyOBO &OntoMF, COntologyOBO &OntoCC, bool fDatabaseIDs,
                            bool fSynonyms) {
 ...
 */
    ...
    bool COntologyOBOImpl::OpenOntology(SParserOBO &sParser) {
        ...
        for (i = 0; i < m_iNodes; ++i) {
            ...
            m_aNodes[i].m_iParents = sParser.m_vecvecstrParents[i].size();
            if (m_aNodes[i].m_iParents) {
                ...
                for (j = 0; j < m_aNodes[i].m_iParents; ++j) {
                    if ((iParent = m_mapNodes[sParser.m_vecvecstrParents[i][j]]) == i) {
                        g_CatSleipnir().error(
                                "COntologyOBOImpl::OpenOntology( ) found a loop for node %d: %s has parent %s",
