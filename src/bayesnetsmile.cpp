...
namespace Sleipnir {
    ...
    bool CBayesNetSmileImpl::IsNaive(const DSL_network &BayesNet) {
        ...
        {
            const DSL_intArray &veciParents = ((DSL_network &) BayesNet).GetParents(0);

            if (veciParents.NumItems() != 0)
                return false;
        }
        for (i = 1; i < BayesNet.GetNumberOfNodes(); ++i) {
            const DSL_intArray &veciParents = ((DSL_network &) BayesNet).GetParents(i);
 ...
 */
    ...
    void CBayesNetSmileImpl::LearnExpected(DSL_node *pNode, DSL_Dmatrix *pExpected,
        ...
        veciParents = this->m_SmileNet.GetParents(pNode->Handle());
        // pNode->Parents( );
            ...
            if (pDefault)
                for (j = 0; j < iAnswers; ++j) {
                    ...
                    if (dCount < c_iMinimum) {
                        g_CatSleipnir().warn(
                                "CBayesNetSmile::LearnNaive( %d ) insufficient data for node %s, column %d",
...
#pragma warning( default : 4996 )
            ...
            if (veciDefaults[i] != -1) {
#pragma warning( disable : 4996 )
                sprintf(acNum, "%lu", veciDefaults[i]);
#pragma warning( default : 4996 )
