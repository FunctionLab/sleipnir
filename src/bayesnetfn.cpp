    ...
    bool CBayesNetFNNodeBeta::Evaluate(float dValue, vector<float> &vecdOut) const {
        ...
        for (i = 0; i < vecdOut.size(); ++i) {
            ...
            dSum += (vecdOut[i] = (float) CStatistics::BetaPDF(d, m_Params.Get(i, c_iMin),
                                                               m_Params.Get(i, c_iMax), m_Params.Get(i, c_iAlpha),
...
// CBayesNetFN //////////////////////////////////////////////////////////////
    ...
    CBayesNetFNImpl::CBayesNetFNImpl() : CBayesNetImpl(true), m_iNodes(0), m_apNodes(nullptr), m_fSmileNet(false) {}
...
// CBayesNetMinimal //////////////////////////////////////////////////////////
    ...
    bool CBayesNetMinimalImpl::Counts2Probs(const std::vector <std::string> &vecstrCounts,
                                            std::vector<float> &vecdProbs, float dAlpha, float dPseudocounts,
        ...
        if ((iTotal < c_iMinimum) && pBNDefault) {
            ...
            if (MatDefault.GetRows() != vecdProbs.size()) {
                g_CatSleipnir().error(
                        "CBayesNetMinimal::Counts2Probs( ) default distribution size mismatch: wanted %d, got %d",
    ...
    float CBayesNetMinimal::Evaluate(const vector<unsigned char> &vecbDatum, size_t iOffset) const {
        ...
        for (i = 0; i < m_vecNodes.size(); ++i) {
            ...
            for (j = 0; j < MatCPT.GetColumns(); ++j) {
                if (c >= MatCPT.GetRows()) {
                    g_CatSleipnir().error("CBayesNetMinimal::Evaluate( %d ) illegal value: %d/%d in %d", iOffset, c,
                                          MatCPT.GetRows(), i);
                    return CMeta::GetNaN();
                }
 ...
 */
    bool CBayesNetMinimal::OpenCounts(const char *szFileCounts, const std::map <std::string, size_t> &mapstriNodes,
                                      ...
                                      const std::vector<float> &vecdAlphas, float dPseudocounts,
                                      const CBayesNetMinimal *pBNDefault) {
                        ...
                        return false;
                    {
                        ...
                        if (iClass) {
                            if (MatCPT.GetRows() != vecdProbs.size()) {
                                g_CatSleipnir().error(
                                        "CBayesNetMinimal::OpenCounts( %s ) illegal count number: given %d, expected %d",
