    ...
    protected:
        ...
        static float GetInformation(const CFullMatrix<uint16_t> &MatPWM) {
            ...
            for (iPos = 1; iPos < MatPWM.GetColumns(); ++iPos)
                for (iFrom = 0; iFrom < MatPWM.GetRows(); ++iFrom) {
                    if (!(bool) (dFrom = ((float) MatPWM.Get(iFrom, iPos - 1) / veciTotals[iPos - 1])))
                        continue;
        ...
        explicit CCoalesceMotifLibraryImpl(size_t iK) : m_iK(iK), m_dPenaltyGap(1), m_dPenaltyMismatch(2) {
            uint32_t i, j, iRC;
