        ...
        if (fBinary) {
            ...
            while (istm.peek() != EOF) {
                ...
                for (iToken = 0; iToken < 3; ++iToken) {
                    ...
                    if (!(bool) d && ((pc - strToken.c_str()) != strToken.length())) {
                        delete[] acBuf;
                        return false;
                    }
                }
 ...
 */
    void CDat::FilterGenes(const CGenes &Genes, EFilter eFilter, size_t iLimit, float dEdgeAggressiveness,
        ...
        switch (eFilter) {
            case EFilterPixie:
            case EFilterHefalmp:
                FilterGenesGraph(Genes, vecfGenes, iLimit, dEdgeAggressiveness, eFilter == EFilterHefalmp, fAbsolute,
                                 pvecdWeights);
                return;
            default:
                break;
        ...
        for (i = 0; i < GetGenes(); ++i) {
            if (((eFilter == EFilterExclude) && vecfGenes[i]) ||
                ...
                for (j = (i + 1); j < GetGenes(); ++j) {
                    if ((bool) Get(i, j) || eFilter != EFilterIncludePos)
                        Set(i, j, CMeta::GetNaN());
                }
                continue;
            }
    ...
    void CDatImpl::FilterGenesGraph(const CGenes &Genes, vector<bool> &vecfGenes, size_t iLimit,
                                    float dEdgeAggressiveness, bool fHefalmp, bool fAbsolute,
