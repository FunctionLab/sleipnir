 ...
 */
    int CPCL::Distance(const char *szFile, size_t iSkip,
        ...
        IMeasure *apMeasures[] = {&Pearson, &EuclideanSig, &KendallsTau,
                                  ...
                                  &InnerProd, &BinInnerProd, &MutualInfo, &RelAuc, &PearSig, &Dice, &DCor, &SDCor,
                                  &Cosine, nullptr};
        ...
        adWeights = vecdWeights.empty() ? nullptr : &vecdWeights[0];
        ...
        if ((iLimit != -1) && (PCL.GetGenes() > iLimit))
            ...
            if (strcmp(pMeasure->GetName(), "pearson") == 0 ||
                strcmp(pMeasure->GetName(), "pearnorm") == 0) {
                ...
                if (strcmp(pMeasure->GetName(), "pearson") == 0) {
                    upper = 1.0;
                    lower = -1.0;
                } else if (strcmp(pMeasure->GetName(), "pearnorm") == 0) {
                    upper = 5.0;
                    lower = -5.0;
                }
 ...
 */
    int CPCL::Distance(const char *szFile, size_t iSkip, const char *szWeights,
        ...
        if ((iLimit != -1) && (PCL.GetGenes() > iLimit))
            ...
            Dat.Open(GenesIn.GetGeneNames());
            g_CatSleipnir().info("first gene is: %s", GenesIn.GetGeneNames().front().c_str());
            for (i = 0; i < Dat.GetGenes(); ++i)
                for (j = (i + 1); j < Dat.GetGenes(); ++j)
                    Dat.Set(i, j, CMeta::GetNaN());
            for (i = 0; i < GenesIn.GetGenes(); ++i) {
...
#pragma omp parallel for num_threads(nThreads)
            ...
            if ((strcmp(pMeasure->GetName(), "pearson") == 0) || (strcmp(pMeasure->GetName(), "pearnorm") == 0)) {
                vector<unsigned long> bins;
                bins.resize(55);
                float upper = 0;
                float lower = 0;
 ...
 */
    ...
    bool CPCLImpl::OpenExperiments(std::istream &istmInput, size_t iFeatures,
        ...
        for (iToken = 0, pc = acStr.c_str(); (strToken = OpenToken(pc, &pc)).length() || *pc; ++iToken)
            if (iToken < iFeatures)
...
//Save only certain genes
    void CPCL::Save(const char *szFile, const std::vector <size_t> *pveciGenes) const {
        ofstream ofsm;
    ...
    void CPCL::Save(std::ostream &ostm, const std::vector <size_t> *pveciGenes) const {
        ...
        SaveHeader(ostm, pveciGenes != nullptr);
            ...
            for (j = 0; j < veciGenes.size(); ++j) {
                ...
                for (k = (j + 1); k < veciGenes.size(); ++k) {
                    iBin = MedianMultiplesBin((float) Euclidean.Measure(Get(iOne),
                                                                        GetExperiments(), Get(veciGenes[k]),
                                                                        GetExperiments(),
                                                                        IMeasure::EMapNone), dAve, dStd, iBins,
                                              dBinSize);
                    //				if( iBin <= iCutoff ) {
