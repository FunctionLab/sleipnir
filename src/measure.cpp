 ...
 */
    double CMeasurePearson::Pearson(const float *__restrict__ adX, size_t iM, const float *__restrict__ adY,
    ...
    double CMeasureKendallsTau::Measure(const float *adX, size_t iM,
        ...
        dRet = (adWX || adWY) ? CMeasureKendallsTauImpl::MeasureWeighted(adX, adY,
                                                                         iN, adWX, adWY)
                              : CMeasureKendallsTauImpl::MeasureUnweighted(adX,
                                                                           adY, iN);
        if (dRet < -1)
            dRet = -1;
        else if (dRet > 1)
 ...
 */
    ...
    double CMeasureMutualInformation::Measure(const float *adX, size_t iM,
        ...
        map < pair < float, float >, size_t > ::iterator
        iterJoint;
