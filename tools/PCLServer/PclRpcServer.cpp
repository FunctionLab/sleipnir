...
{
    // parse options
    string usage = "Usage: pclRpcService -c <species_1.toml> [-c <species_2.toml> ...]\n"
                   "Starts the PclService to handle RPC requests. " 
...
int main(int argc, char** argv) 
{
    ...
    try {
        ...
        PclInterface pclInterface(args.configFiles, args.maxThreads, args.taskTimeoutSec);

        bool startServer = true;
        if (startServer == true) {
            shared_ptr<PclRPCHandler> handler(new PclRPCHandler(pclInterface));
            shared_ptr<TProcessor> processor(new PclRPCProcessor(handler));
