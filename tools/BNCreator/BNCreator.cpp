...
void *learn(void *pData) {
    ...
    for (i = 0; i < psData->m_pvecpsOutputs->size(); ++i) {
        ...
        if (!(*psData->m_pvecpsOutputs)[i]->LearnNode(psData->m_iNode, *psData->m_pAnswers, &Data, vecstrNames,
                                                      veciZeros,
                                                      psData->m_psArgs->default_arg ? psData->m_pBNDefault : NULL,
                                                      !!psData->m_psArgs->zero_flag))
            return NULL;
    }
