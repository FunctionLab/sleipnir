...
int main(int iArgs, char **aszArgs) {
    ...
    for (i = 0; i < PCL.GetGenes(); ++i) {
        ...
        for (iGene = j = 0; j < PCL.GetGenes(); ++j)
            if (!vecfClinical[j]) {
                vecdScores[iGene] = (float) CMeasurePearson::Pearson(PCL.Get(i), PCL.GetExperiments(), PCL.Get(j),
                                                                     PCL.GetExperiments(),
                                                                     IMeasure::EMapNone, NULL, NULL,
                                                                     &veciScores[iGene]);
                iGene++;
            }
        ...
        if (sArgs.global_arg && sArgs.initial_arg) {
            ...
            DatCopy.Open(Dat);
            cerr << "Filtering...";
            DatCopy.FilterGenes(GenesInitial, sArgs.hefalmp_flag ? CDat::EFilterHefalmp : CDat::EFilterPixie,
                                sArgs.final_arg, CMeta::GetNaN());
            cerr << "  done!" << endl;
        ...
        for (j = 0; j < veciFinal.size(); ++j) {
            ...
            d = (float) (sArgs.spearman_flag ? CStatistics::PValueSpearman : CStatistics::PValuePearson)(vecdScores[k],
                                                                                                         veciScores[k]);
            cout << PCL.GetGene(i) << '\t' << PCL.GetGene(veciFinal[j]) << '\t' << vecdScores[k] << '\t'
                 << veciScores[k] << '\t' <<
                 d << '\t' << (d * iGene) << endl;
        }
    }
