/*
  ...
  ../gengetopt-2.13.1/bin/gengetopt -u -N -e -i Txt2Bin.ggo 
...
char *from_values[] = {"txt", "dat", "bin", 0} ;	/* Possible values for from.  */
char *to_values[] = {"txt", "bin", 0} ;	/* Possible values for to.  */
...
void clear_args (struct gengetopt_args_info *args_info)
{
  ...
  args_info->from_arg = gengetopt_strdup ("txt");
...
cmdline_parser_print_help (void)
{
  ...
  printf("\n%s\n", "SVM binary input manipulator");
  printf("\nUsage: Txt2Bin [OPTIONS]... [FILES]...\n\n");
  printf("%s\n","  -h, --help              Print help and exit");
  printf("%s\n","  -V, --version           Print version and exit");
  printf("%s\n","\nOptional:");
  printf("%s\n","  -i, --input=filename    Input text/binary file");
  printf("%s\n","  -o, --output=filename   Output binary file");
...
cmdline_parser_free (struct gengetopt_args_info *args_info)
{
  ...
  if (args_info->from_arg)
    {
      free (args_info->from_arg); /* free previous argument */
      args_info->from_arg = 0;
    }
  if (args_info->to_arg)
    {
      free (args_info->to_arg); /* free previous argument */
      args_info->to_arg = 0;
    }
  ...
  while (1)
    {
      ...
      stop_char = 0;
      c = getopt_long (argc, argv, "hVi:o:v:f:t:g:G:w:", long_options, &option_index);
      ...
      switch (c)
        ...
        case 'f':	/* Source format.  */
          if (local_args_info.from_given)
            {
              fprintf (stderr, "%s: `--from' (`-f') option given more than once%s\n", argv[0], (additional_error ? additional_error : ""));
              goto failure;
            }
          if ((found = check_possible_values(optarg, from_values)) != 1)
            {
              fprintf (stderr, "%s: %s argument, \"%s\", for option `--from' (`-f')%s\n", argv[0], found ? "ambiguous" : "invalid", optarg, (additional_error ? additional_error : ""));
              goto failure;
            }
          if (args_info->from_given && ! override)
            continue;
          local_args_info.from_given = 1;
          args_info->from_given = 1;
          if (args_info->from_arg)
            free (args_info->from_arg); /* free previous string */
          args_info->from_arg = gengetopt_strdup (optarg);
          break;

        case 't':	/* Target format.  */
          if (local_args_info.to_given)
            {
              fprintf (stderr, "%s: `--to' (`-t') option given more than once%s\n", argv[0], (additional_error ? additional_error : ""));
              goto failure;
            }
          if ((found = check_possible_values(optarg, to_values)) != 1)
            {
              fprintf (stderr, "%s: %s argument, \"%s\", for option `--to' (`-t')%s\n", argv[0], found ? "ambiguous" : "invalid", optarg, (additional_error ? additional_error : ""));
              goto failure;
            }
          if (args_info->to_given && ! override)
            continue;
          local_args_info.to_given = 1;
          args_info->to_given = 1;
          if (args_info->to_arg)
            free (args_info->to_arg); /* free previous string */
