...
struct SDatum {
    ...
    SDatum(float dAnswer, float dData, size_t iOne, size_t iTwo) : m_dData(dData), m_dAnswer(dAnswer), m_iOne(iOne),
                                                                   m_iTwo(iTwo) {
...
//		sArgs.everything_flag = true;
    ...
    if (!Answers.Open(sArgs.answers_arg,
                      sArgs.memmap_flag && !sArgs.genes_arg && !sArgs.genet_arg && !sArgs.genex_arg)) {
            ...
            if (pOne)
                for (j = 0; j < pOne->GetOntologies(); ++j)
                    if (pOne->GetAnnotations(j) && ((pOne->GetAnnotations(j) > 1) ||
                                                    (pOne->GetOntology(j)->GetID(pOne->GetAnnotation(j, 0)) !=
                                                     c_szUnknown))) {
                        fOne = false;
                        break;
                    }
            if (pTwo)
                for (j = 0; j < pTwo->GetOntologies(); ++j)
                    if (pTwo->GetAnnotations(j) && ((pTwo->GetAnnotations(j) > 1) ||
                                                    (pTwo->GetOntology(j)->GetID(pTwo->GetAnnotation(j, 0)) !=
                                                     c_szUnknown))) {
                        fTwo = false;
                        break;
                    }
