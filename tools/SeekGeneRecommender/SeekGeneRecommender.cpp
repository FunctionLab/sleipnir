...
if(
CSeekTools::IsNaN(mapG
->
GetForward(i)
)) continue;
tot+=mat[d][0].

size(); //mat[d][0].size() is the number of conditions
}
if(tot==0){
absent_g[i] = 1;
continue;
...
bool weight_experiment(
        ...
        int & numExperiments, float
cut_off_percentage,
vector <utype> &q, vector<bresult>
&a){
...
push_back(j);
array_f[tid].
push_back(significance);
array_f2[tid].
push_back(mean_query);
...
vector<CSeekIntIntMap *> &dm,
        vector<bresult>
&a,
int numExperiments, vector<char>
&absent_g,
float cut_off_percentage, vector<float>
&gs){

int i, ii, d, k;
CSeekTools::InitVector(gs, numGenes,
(float) -320);
vector<int> tot;
CSeekTools::InitVector(tot, numGenes,
(int) 0);
...
size(); //numConditions in a dataset
}
}
