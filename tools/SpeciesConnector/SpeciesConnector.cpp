...
int main(int iArgs, char **aszArgs) {
    ...
    for (i = 0; i < vecstrInputs.size(); ++i) {
        if (strcmp(&vecstrInputs[i][vecstrInputs[i].rfind(".") + 1], "xdsl") == 0) {
            ...
            vecstrxInputs[i] = vecstrInputs[i].substr(vecstrInputs[i].rfind("/") + 1,
                                                      vecstrInputs[i].rfind(".") - vecstrInputs[i].rfind("/") - 1);
        } else {
    ...
    if (sArgs.learn_flag) {
        ...
        for (i = 0; i < vecvecSpDat.size(); ++i) {
            ...
            for (j = 0; j < vecvecSpDat[i].size(); ++j) {
                vecvecstrInputs[i][j] =
                        (string) sArgs.ddirectory_arg + '/' + vecstrxInputs[i] + '/' + vecvecSpDat[i][j] + c_acDab;
                countstrInputs++;
            }
        }
            ...
            for (j = 0; j < vecvecstrInputs[i].size(); ++j) {
                ...
                for (iDatOne = 0; iDatOne < vecstrFGenes.size(); ++iDatOne) {
                    for (iDatTwo = (iDatOne + 1); iDatTwo < vecstrFGenes.size(); ++iDatTwo) {
                        ...
                        float DFValue;
                        DFValue = ((iGeneOne == -1) || (iGeneTwo == -1)) ? CMeta::GetNaN() : DataF.Get(iGeneOne,
                                                                                                       iGeneTwo);
            ...
            for (i = 0; i < vecstrxInputs.size(); ++i) {
                for (iDatOne = 0; iDatOne < vecstrFGenes.size(); ++iDatOne) {
                    for (iDatTwo = (iDatOne + 1); iDatTwo < vecstrFGenes.size(); ++iDatTwo) {
                        ...
                        for (k = 0; k < vecstrxInputs.size(); ++k) {
                            if (k != i) {
                                sumOne += log(exp(log(vec4OSpGSp[i][k][1][0]) +
                                                  DataZero[k]->Get(veciGenesZero[k][iDatOne],
                                                                   veciGenesZero[k][iDatTwo])) +
                                              exp(log(vec4OSpGSp[i][k][1][1]) +
                                                  DataOne[k]->Get(veciGenesOne[k][iDatOne], veciGenesOne[k][iDatTwo])));
                                sumZero += log(exp(log(vec4OSpGSp[i][k][0][0]) +
                                                   DataZero[k]->Get(veciGenesZero[k][iDatOne],
                                                                    veciGenesZero[k][iDatTwo])) +
                                               exp(log(vec4OSpGSp[i][k][0][1]) +
                                                   DataOne[k]->Get(veciGenesOne[k][iDatOne],
                                                                   veciGenesOne[k][iDatTwo])));
                            }
                        }
                        sumOne += (DataOne[i]->Get(veciGenesOne[i][iDatOne], veciGenesOne[i][iDatTwo]) +
                                   log(vecPrior[i][1]));
                        sumZero += (DataZero[i]->Get(veciGenesZero[i][iDatOne], veciGenesZero[i][iDatTwo]) +
                                    log(vecPrior[i][0]));
