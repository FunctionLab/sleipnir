/*
  ...
  ..\gengetopt-2.13.1\Release\gengetopt.exe -N -e -i Clusters2Dab.ggo 
...
char *type_values[] = {"samba", "list", "param", "fuzzy", 0} ;	/* Possible values for type.  */
...
cmdline_parser_print_help (void)
{
  ...
  printf("\n%s\n", "Generate pairwise scores from preclustered output.");
  printf("\nUsage: Clusters2Dab [OPTIONS]...\n\n");
  ...
  while (1)
    {
      ...
      stop_char = 0;
      c = getopt_long (argc, argv, "hVo:i:t:s:v:c", long_options, &option_index);
      ...
      switch (c)
        ...
        case 'c':	/* Calculate pair weight by cocluster frequency.  */
          if (local_args_info.counts_given)
            {
              fprintf (stderr, "%s: `--counts' (`-c') option given more than once%s\n", argv[0], (additional_error ? additional_error : ""));
              goto failure;
            }
          if (args_info->counts_given && ! override)
            continue;
          local_args_info.counts_given = 1;
          args_info->counts_given = 1;
          args_info->counts_flag = !(args_info->counts_flag);
          break;
